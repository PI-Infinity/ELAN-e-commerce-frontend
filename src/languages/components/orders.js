...
function Orders(props) {
  ...
  const itemName = useSelector((state) => state.storeOrder.itemName);
  const qnt = useSelector((state) => state.storeOrder.qnt);
  const itemPrice = useSelector((state) => state.storeOrder.itemPrice);
  ...
  const addItemInList = () => {
    if (itemList?.find((item) => item?.name === itemName) == undefined) {
      setItemList([
        ...itemList,
        { name: itemName, quantity: qnt, price: itemPrice },
      ]);
    } else {
  ...
  async function AddOrderToFirebase() {
    ...
    const N = await num.toString();
    const orderList2 = collection(db, `orders/${num}/items`);
    await itemList?.map((item) => {
      setDoc(doc(orderList2, item?.name), {
        name: item?.name,
  ...
  const HandleSubmit = () => {
    if (
      (buyerName?.length > 0 &&
        ...
        // adress?.length > 0 &&
        delivery?.length > 0) ||
      (delivery.length === 0 && bank?.length > 0)
    ) {
      AddOrderToFirebase();
      alert("added");
    } else {
    ...
    return orders
      ...
      ?.filter((item) => {
        if (sortBank == "") {
          ...
          item?.items[0]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[1]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[2]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[3]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[4]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[5]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[6]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[7]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[8]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[9]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[10]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[12]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[13]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[14]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[15]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[16]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[17]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[18]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[19]?.toLowerCase()?.includes(sortBank?.toLowerCase()) ||
          item?.items[20]?.toLowerCase()?.includes(sortBank?.toLowerCase())
        ) {
          return item;
        }
      });
      ...
      >
        ...
        >
            ...
            >
              <Item style={{ display: "flex", alignItems: "center" }}>
                ...
                <Input
                  ...
                  onChange={(e) => dispatch(setQnt(e.target.value))}
                />
                <Input
                  type="number"
                  value={itemPrice}
          ...
          >
            <Input
              type="text"
              placeholder="Comment"
              style={{ width: "70%", boxShadow: "0px 1px 3px rgba(0,0,0,0.1)" }}
        ...
        >
          ...
          <div>
            <span>Income: </span>
            <b style={{ color: "green" }}>
              {(parseFloat(definedSum) + parseFloat(deliverySumm)).toFixed(2)}{" "}
              Gel
            </b>
          </div>
          <div>
          ...
          {/* <div>
            <span>Outcome: </span>
            <b style={{ color: "red" }}>
              {(parseFloat(outcomeSum) + parseFloat(discountSum)).toFixed(2)}{" "}
              Gel
            </b>
          </div> */}
          <div>
          ...
          >
            <span style={{ fontSize: "14px", marginRight: "10px" }}>End: </span>
            <EndCalendarComponent />
          </div>
        </div>
        <div
